...
public class Main extends Application {
	...
	public void start(Stage primaryStage) {
		// TODO Replace with T.K's GUI code
				...
				// when login button, create User(info) with name, street, city state, and zipcode, and then login(user)
				// when register new user, create User(info) with name, street, city state, and zipcode, and then call login(user))
				//        If login(info) fails (is false), continue to get covid status, interactions(exact input from user), and call registerUser(user, status, interactions)
